...
// Builds tag_c — TagLib's C binding + our extra helpers — as ONE shared
// library for both platforms:
//   zig build -Drelease                       -> lib/libtag_c.so
//   zig build -Drelease -Dtarget=x86_64-windows-gnu -> bin/tag_c.dll
// (zig adds "lib" to the ELF name only; PE DLLs keep their bare name.)
// C++ runtime is linked statically in; zlib is off, identically on both.
// CI (.github/workflows/libtag_c.yml) runs both targets on tags taiko-*
// and attaches the artifacts to the GitHub release.
//
// Sources and include dirs are discovered by walking the tree, so upstream
// files can be added or renamed without touching this file. The two headers
// CMake would generate (config.h, taglib_config.h) are declared below via
// addConfigHeader — no committed "gen/", no CMake needed.
...
pub fn build(b: *std.Build) void {
    ...
    // Headers CMake would generate: config.h (only under HAVE_CONFIG_H),
    // taglib_config.h (unconditional). zlib off: compressed ID3v2 frames
    // are a rare legacy case and both platforms behave identically.
    ...
    // Stock installArtifact: ELF shared libs land in lib/, DLLs in bin/.
